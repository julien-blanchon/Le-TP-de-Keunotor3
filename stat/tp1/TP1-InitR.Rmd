...
Ce script s'appuie sur le tutoriel 2 ["Initiation au langage et objets de R "](https://cmaugis.github.io/TutorielsR/Part2-InitR.html). Il vous donne un guide de lecture du tutoriel et il y est reporté des parties de code.
...
-   le vecteur $d$ contenant la somme des éléments de $c$, puis la longueur de $b$, puis 5 fois le chiffre 3.
    ...
    c = seq(from=4, to=30, length=6)

    d = c(sum(c), length(b), 5*3)
    ```
...
```{r}
# Unique supprime les duplication d'elt dans un vecteur
unique(c(2, 3, 2))
...
```{r}
A = matrix(data = c(rep(3,3), rep(4,3), rep(5,3)), ncol=3, byrow=T)
A+2
...
```{r}
l = list("Matiere" = c("Math", "Physique", "Français"), "notes"=atrix(data=1:15, ncol=3, dimnames = list(NULL, c("Julien", "Jule", "Julia"))) )
```
...
4.  Ajouter la tangente en zéro en pointillés rouge
    ...
    par()
    # premier graphique
    plot(x, atan(x), type = "l", col = 1, main = "arctan")
    abline(h = -pi/2, col='blue')
    abline(h = pi/2, col='blue')
    points(-pi, atan(-pi), pch = "o", cex = 3, col = "green")
    points(0, atan(0), pch = "o", cex = 3, col = "green")
    points(pi, atan(pi), pch = "o", cex = 3, col = "green")
...
Saurez-vous tracer la "tête à Toto" ? Voir l'image à recréer sur le tutoriel.
...
```{r}
mafunc = function(x, y) {
    res = round(x**2 + y, digit=3)
    return(res)
}

mafunc2 = function(x, m=0, sigma2=1) {
  res = (1/sqrt(2*pi*sigma2))*exp( ((x-m)**2)/(2*sigma2) )
  return(res)
}
```
...
```{r}
p_poisson = function(k, lambda=2) {
    res = (exp(-lambda)*lambda**k)/factorial(k)
    return(res)
}
```
